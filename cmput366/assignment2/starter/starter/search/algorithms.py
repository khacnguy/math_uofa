...
class CBSState:
    ...
    def is_solution(self):
        ...
        for agent in range(self._k):
            for time in range(len(self._paths[agent])):
                state = self._paths[agent][time]
                if (state, time) in temp:
                    return False, (state, state.get_g())

                temp.append((state, time))
        return True, None

            
    def successors(self, conflict):
        agent_conflict_list = []
        for i in range(self._k):
            if len(self._paths[i])-1 >= conflict[1] and self._paths[i][conflict[1]] == conflict[0]:
                agent_conflict_list.append(i)
        successors_list = []
        for i in range(2):
            c = CBSState(self._map, self._starts, self._goals)
            c._constraints = copy.deepcopy(self._constraints)
            c.set_constraint(conflict[0], conflict[1], agent_conflict_list[i])
            successors_list.append(c)
...
class CBS():
    def search(self, start):
        ...
        open_list = []
        start.compute_cost()
        # not needed since every cases have a solution
        # if start._cost == -1:
        #     return 
        ...
        while len(open_list) > 0:
            ...
            is_sol, conflict = current_state.is_solution()
            if is_sol:
                return current_state._paths, current_state._cost
            for neighbor_state in current_state.successors(conflict):
                neighbor_state.compute_cost()
                if neighbor_state._cost != -1:
                    heapq.heappush(open_list, neighbor_state)
        return None, -1

        # there is always a solution, do not need
