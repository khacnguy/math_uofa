...
import getopt
import sys
from search.algorithms import dijkstra_alg, astar_alg
...
def main():
    ...
    for i in range(0, len(start_states)):    
        ...
        cost, expanded_diskstra = dijkstra_alg(gridded_map, start, goal) # replace None, None with the call to your Dijkstra's implementation
        # cost, expanded_diskstra = None, None
        time_end = time.time()
        ...
        if cost != solution_costs[i]:
            ...
            print("no mismatchhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh") 

        start = start_states[i]
        goal = goal_states[i]
    
        time_start = time.time()
        cost, expanded_astar = astar_alg(gridded_map, start, goal) # replace None, None with the call to your A* implementation
        time_end = time.time()
